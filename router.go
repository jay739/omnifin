...
func (app *appContext) loadRoutes(router *gin.Engine) {
	...
	for _, p := range routePrefixes {
		...
		api.GET(p+"/users/announce/scheduled", app.ListScheduledAnnouncements)
		api.POST(p+"/users/announce/scheduled", app.CreateScheduledAnnouncement)
		api.DELETE(p+"/users/announce/scheduled/:id", app.DeleteScheduledAnnouncement)
