...
func (mb *MediaBrowser) get(url string, params map[string]string, queryParams map[string]interface{}) (string, int, error) {
	...
	resp, err := mb.httpClient.Do(req)
	defer mb.timeoutHandler()
	// LOCAL PATCH (2026-07-16, see ../omnifin project memory project_omnifin.md
	// for the incident this fixes): resp is nil whenever Do() fails outright
	// (DNS failure, connection refused -- e.g. Jellyfin's container being
	// stopped), and the original code unconditionally deferred
	// resp.Body.Close() before ever checking that, panicking with a nil
	// pointer dereference on every single call made while Jellyfin is
	// unreachable. This one bug was the true root cause of daemon/cache-sync
	// panics across many call sites in the consuming app, since get()/post()
	// are the shared HTTP primitives underlying nearly every method in this
	// package.
	if resp == nil {
		return "", 0, err
	}
...
func (mb *MediaBrowser) post(url string, data interface{}, response bool) (string, int, error) {
	...
	resp, err := mb.httpClient.Do(req)
	defer mb.timeoutHandler()
	// LOCAL PATCH (2026-07-16): see the matching comment in get() above --
	// same nil-resp panic, same fix.
	if resp == nil {
		return "", 0, err
	}
	defer resp.Body.Close()
